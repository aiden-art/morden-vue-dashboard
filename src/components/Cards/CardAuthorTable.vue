    ...
    <a-table :columns="columns" :data-source="data" :pagination="false">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'status'">
          <a-tag
            class="tag-status"
            :class="record.status ? 'ant-tag-primary' : 'ant-tag-muted'"
          >
            {{ record.status ? "ONLINE" : "OFFLINE" }}
          </a-tag>
        </template>
        <template v-if="column.dataIndex === 'editBtn'">
          <a-button type="link" :data-id="record.key" class="btn-edit">
            Edit
          </a-button>
        </template>
      </template>
...
const authorsHeaderBtns = ref("all");
</script>
